...
#define FT_SG                                                                  \
  2                   /* filetype is sg char device or supports                \
                         SG_IO ioctl */
...
#define FT_RANDOM_0_FF                                                         \
  256                /* iflag=00, iflag=ff and iflag=random                    \
                        overriding if=IFILE */
...
static void find_bsg_major(void) {
  int n;
  char *cp;
  FILE *fp;
  ...
  while ((cp = fgets(b, sizeof(b), fp))) {
    if ((1 == sscanf(b, "%126s", a)) && (0 == memcmp(a, "Character", 9)))
      break;
  }
...
static void find_nvme_major(void) {
  int n;
  char *cp;
  FILE *fp;
  const char *proc_devices = "/proc/devices";
...
static int scsi_read_capacity(int sg_fd, int64_t *num_sect, int *sect_sz) {
  ...
  res =
      sg_ll_readcap_10(sg_fd, false, 0, rcBuff, READ_CAP_REPLY_LEN, true, verb);
  if (0 != res)
    return res;
  ...
  if ((0xff == rcBuff[0]) && (0xff == rcBuff[1]) && (0xff == rcBuff[2]) &&
    ...
    res =
        sg_ll_readcap_16(sg_fd, false, 0, rcBuff, RCAP16_REPLY_LEN, true, verb);
    if (0 != res)
      return res;
...
static int sg_build_scsi_cdb(uint8_t *cdbp, int cdb_sz, unsigned int blocks,
                             ...
                             bool write_true, bool fua, bool dpo, int cdl) {
  int sz_ind;
...
static int sg_read_low(int sg_fd, uint8_t *buff, int blocks, int64_t from_block,
  ...
  if (sg_build_scsi_cdb(rdCmd, ifp->cdbsz, blocks, from_block, do_verify, false,
                        ifp->fua, ifp->dpo, ifp->cdl)) {
  ...
  case SG_LIB_CAT_ILLEGAL_REQ:
    if (5 == ifp->pdt) { /* MMC READs can go down this path */
      ...
      if (verbose > 1)
        sg_chk_n_print3("reading", &io_hdr, verbose > 1);
      if (sg_scsi_normalize_sense(sbp, slen, &ssh) && (0x64 == ssh.asc) &&
          (0x0 == ssh.ascq)) {
        if (sg_get_sense_filemark_eom_ili(sbp, slen, NULL, NULL, &ili) && ili) {
...
static int sg_read(int sg_fd, uint8_t *buff, int blocks, int64_t from_block,
  ...
  for (xferred = 0, blks = blocks, lba = from_block, bp = buff; blks > 0;
       blks = blocks - xferred) {
    io_addr = 0;
    repeat = false;
    may_coe = false;
    ...
    if (repeat)
      continue;
    if ((io_addr < (uint64_t)lba) || (io_addr >= (uint64_t)(lba + blks))) {
    ...
    if ((0 != ifp->pdt) || (ifp->coe < 2)) {
      ...
      corrct = (ifp->coe > 2);
      res = sg_ll_read_long10(sg_fd, /* pblock */ false, corrct, lba, buffp,
                              bs + read_long_blk_inc, &offset, true, verbose);
      ok = false;
      switch (res) {
      case 0:
        ok = true;
      ...
      case SG_LIB_CAT_ILLEGAL_REQ_WITH_INFO:
        ...
        r = sg_ll_read_long10(sg_fd, false, corrct, lba, buffp, nl, &offset,
                              true, verbose);
        if (0 == r) {
          ok = true;
          ++read_longs;
...
static int sg_write(int sg_fd, uint8_t *buff, int blocks, int64_t to_block,
  ...
  if (sg_build_scsi_cdb(wrCmd, ofp->cdbsz, blocks, to_block, do_verify, true,
                        ofp->fua, ofp->dpo, ofp->cdl)) {
  ...
  case SG_LIB_CAT_RECOVERED:
    ...
    if (info_valid) {
      ...
      pr2serr("Recovered error: [no info] %s to block=0x%" PRIx64 ", num=%d\n",
...
static void calc_duration_throughput(bool contin) {
  ...
  if (start_tm_valid && (start_tm.tv_sec || start_tm.tv_usec)) {
    ...
    pr2serr("time to transfer data%s: %d.%06d secs", (contin ? " so far" : ""),
 ...
 * 1 on error. */
static int process_conv(const char *arg, struct flags_t *ifp,
                        struct flags_t *ofp) {
  char buff[256];
  char *cp;
  char *np;
 ...
 */
static int open_if(const char *inf, int64_t skip, int bpt, struct flags_t *ifp,
                   int *in_typep, int vb) {
  int infd = -1;
  int flags, fl, t, res;
  char ebuff[EBUFF_SZ];
  ...
  *in_typep = dd_filetype(inf);
  ...
  if (FT_ST & *in_typep) {
    ...
    if ((infd = open(inf, fl | flags)) < 0) {
      fl = O_RDONLY;
      if ((infd = open(inf, fl | flags)) < 0) {
        snprintf(ebuff, EBUFF_SZ, ME "could not open %s for sg reading", inf);
        perror(ebuff);
        goto file_err;
      }
    }
    ...
    if (infd < 0) {
      ...
      if (skip > 0) {
        ...
        if (vb)
          pr2serr("  >> skip: lseek64 SEEK_SET, byte offset=0x%" PRIx64 "\n",
                  (uint64_t)offset);
      }
 ...
 * (-SG_LIB_FILE_ERROR or -SG_LIB_CAT_OTHER) if error.
 */
static int open_of(const char *outf, int64_t seek, int bpt, struct flags_t *ofp,
  ...
  *out_typep = dd_filetype(outf);
  ...
  if (FT_ST & *out_typep) {
    ...
    if ((outfd = open(outf, flags)) < 0) {
      snprintf(ebuff, EBUFF_SZ, ME "could not open %s for sg writing", outf);
      perror(ebuff);
      goto file_err;
    }
    if (vb)
    ...
    if (ofp->dsync)
      flags |= O_SYNC;
    if ((outfd = open(outf, flags)) < 0) {
      snprintf(ebuff, EBUFF_SZ, ME "could not open %s for raw writing", outf);
      perror(ebuff);
      goto file_err;
    }
    ...
    if ((outfd = open(outf, flags, 0666)) < 0) {
      snprintf(ebuff, EBUFF_SZ, ME "could not open %s for writing", outf);
      perror(ebuff);
      goto file_err;
    }
    if (vb)
    ...
    if (seek > 0) {
      ...
      if (lseek64(outfd, offset, SEEK_SET) < 0) {
        snprintf(ebuff, EBUFF_SZ, ME "couldn't seek to required position on %s",
                 outf);
        perror(ebuff);
        goto file_err;
      }
...
static bool check_progress(void) {
...
int main(int argc, char *argv[]) {
  ...
  for (k = 1; k < argc; k++) {
    ...
    if (0 == strncmp(key, "app", 3)) {
      ...
      ++dry_run;
    else if ((0 == strncmp(key, "--help", 6)) || (0 == strcmp(key, "-?"))) {
      usage();
      return 0;
  ...
  if (out2f[0]) {
    ...
    if ((out2fd = open(out2f, O_WRONLY | O_CREAT, 0666)) < 0) {
      res = errno;
      snprintf(ebuff, EBUFF_SZ, ME "could not open %s for writing", out2f);
      perror(ebuff);
      return res;
    }
  } else
  ...
  if ((dd_count < 0) || ((verbose > 0) && (0 == dd_count))) {
    ...
    if (FT_SG & out_type) {
      ...
      if (0 != read_blkdev_capacity(outfd, &out_num_sect, &out_sect_sz)) {
        pr2serr("Unable to read block capacity on %s\n", outf);
        out_num_sect = -1;
      } else if (blk_sz != out_sect_sz) {
        pr2serr("logical block size on %s confusion: bs=%d, device "
