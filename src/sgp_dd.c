...
typedef struct thread_arg { /* pointer to this argument passed to thread */
  int id;
  Rq_coll *clp;
} Thread_arg;
...
static unsigned int GET_NEXT_PACK_ID(unsigned int val) {
  int res;
...
static void calc_duration_throughput(int contin) {
  ...
  pr2serr("time to transfer data %s %d.%06d secs", (contin ? "so far" : "was"),
...
static void thread_exit_handler(int sig) { pthread_exit(0); }
#endif
 ...
 * to __func__ */
#define err_exit(code, text)                                                   \
  do {                                                                         \
    char strerr_buff[STRERR_BUFF_LEN + 1];                                     \
    pr2serr("%s at \"%s\":%d: %s\n", text, __func__, __LINE__,                 \
            tsafe_strerror(code, strerr_buff));                                \
    exit(1);                                                                   \
  } while (0)
...
static int sgp_mem_mmap(int fd, int res_sz, uint8_t **mmpp) {
  int t;
  ...
  *mmpp =
      (uint8_t *)mmap(NULL, res_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (MAP_FAILED == *mmpp) {
    perror("mmap() failed");
    return -1;
  }
...
static int scsi_read_capacity(int sg_fd, int64_t *num_sect, int *sect_sz) {
  ...
  if ((0xff == rcBuff[0]) && (0xff == rcBuff[1]) && (0xff == rcBuff[2]) &&
    ...
    res = sg_ll_readcap_16(sg_fd, 0, 0, rcBuff, RCAP16_REPLY_LEN, false, 0);
    if (0 != res)
      return res;
...
static int sg_in_open(const char *fnp, struct flags_t *flagp, int bs, int bpt) {
  int flags = O_RDWR;
  int fd, err;
  char ebuff[800];
...
static int sg_out_open(const char *fnp, struct flags_t *flagp, int bs,
                       int bpt) {
  int flags = O_RDWR;
  int fd, err;
  char ebuff[800];
...
static void *read_write_thread(void *v_tap) {
  ...
  if ((clp->num_threads > 1) && clp->mmap_active) {
    ...
    if (clp->in_flags.mmap && (FT_SG == clp->in_type)) {
      ...
      if (rep->infd < 0)
        err_exit(-rep->infd, "error opening infn");
    } else
      rep->infd = clp->infd;
    if (clp->out_flags.mmap && (FT_SG == clp->out_type)) {
      rep->outfd = sg_out_open(outfn, &clp->out_flags, clp->bs, clp->bpt);
      if (rep->outfd < 0)
        err_exit(-rep->outfd, "error opening outfn");
  ...
  if (clp->mmap_active) {
    ...
    if (status)
      err_exit(status, "sgp_mem_mmap() failed");
  } else {
...
static void normal_out_operation(Rq_coll *clp, Rq_elem *rep, int blocks) {
  ...
  while (((res = write(rep->outfd, rep->buffp, rep->num_blks * clp->bs)) < 0) &&
         ((EINTR == errno) || (EAGAIN == errno)))
    ;
  if (res < 0) {
...
static int sg_start_io(Rq_elem *rep) {
  ...
  if (sg_build_scsi_cdb(rep->cdb, cdbsz, rep->num_blks, rep->blk, rep->wr, fua,
                        dpo)) {
...
static int sg_finish_io(bool wr, Rq_elem *rep, pthread_mutex_t *a_mutp) {
  ...
  case SG_LIB_CAT_RECOVERED:
    sg_chk_n_print3((wr ? "writing continuing" : "reading continuing"), hp,
                    false);
    break;
  case SG_LIB_CAT_ABORTED_COMMAND:
  case SG_LIB_CAT_UNIT_ATTENTION:
  ...
  default: {
    ...
    snprintf(ebuff, EBUFF_SZ, "%s blk=%" PRId64, wr ? "writing" : "reading",
             rep->blk);
...
int main(int argc, char *argv[]) {
  ...
  if (infn[0] && ('-' != infn[0])) {
    ...
    if (FT_ERROR == clp->in_type) {
      ...
      if ((clp->infd = open(infn, flags)) < 0) {
        err = errno;
        snprintf(ebuff, EBUFF_SZ, "%scould not open %s for reading", my_name,
                 infn);
        perror(ebuff);
        return sg_convert_errno(err);
      } else if (skip > 0) {
  ...
  if (outfn[0] && ('-' != outfn[0])) {
    ...
    if (FT_ST == clp->out_type) {
      ...
      clp->outfd = sg_out_open(outfn, &clp->out_flags, clp->bs, clp->bpt);
      if (clp->outfd < 0)
        return -clp->outfd;
    } else if (FT_DEV_NULL == clp->out_type)
  ...
  if (dd_count < 0) {
    ...
    if (FT_SG == clp->in_type) {
      ...
      if (0 != read_blkdev_capacity(clp->infd, &in_num_sect, &in_sect_sz)) {
    ...
    if (FT_SG == clp->out_type) {
      ...
      if (0 != read_blkdev_capacity(clp->outfd, &out_num_sect, &out_sect_sz)) {
        pr2serr("Unable to read block capacity on %s\n", outfn);
        out_num_sect = -1;
      }
  ...
  status = pthread_create(&sig_listen_thread_id, NULL, sig_listen_thread,
                          (void *)clp);
  if (0 != status)
